...
resource "azurerm_storage_account" "blob_storage_account" {
  ...
  name                     = lower("${local.resource_prefix}-${var.blob_storage_account_name}${random_string.myrandom.id}")
...
resource "azurerm_storage_container" "blob_storage_container" {
  ...
  name                  = lower("${local.resource_prefix}-${var.blob_storage_container_names[count.index]}")
